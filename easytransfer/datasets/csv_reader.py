...
class CSVReader(Reader):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(CSVReader, self).__init__(
            batch_size, is_training, thread_num, input_queue, output_queue, job_name, **kwargs
        )
        ...
        if is_training:
            ...
            tf.compat.v1.logging.info(
                "{}, total number of training examples {}".format(input_glob, self.num_train_examples)
            )
        else:
            with tf.io.gfile.GFile(input_glob, "r") as f:
                for record in f:
    ...
    def _decode_csv(self, record):
        ...
        items = tf.io.decode_csv(
            records=record, field_delim="\t", record_defaults=record_defaults, use_quote_delim=False
        )
        outputs = dict()
        total_shape = 0
        for shape in shapes:
                ...
                if sum(feature.shape) > 1:
                    ...
                    if default_value[0] == "":
                        ...
                        output = tf.strings.to_number(
                            ...
                            tf.strings.split(tf.expand_dims(input_tensor, axis=0), sep=",").values,
                            feature.dtype,
                        )
                        ...
                        output = tf.reshape(
                            tf.io.decode_raw(decode_b64_data, out_type=tf.float32),
                            [
                                feature.shape[0],
                            ],
                        )
            ...
            if self.is_training:
                ...
                d = d.apply(
                    ...
                    tf.data.experimental.parallel_interleave(
                        tf.data.TextLineDataset, sloppy=True, cycle_length=cycle_length  # sloppy 允许是乱序输出
                    )
                )
                ...
                d = d.apply(
                    tf.data.experimental.parallel_interleave(
                        tf.data.TextLineDataset, sloppy=True, cycle_length=cycle_length
                    )
                )

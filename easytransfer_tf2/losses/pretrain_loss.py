...
def next_sentence_prediction_loss(nsp_logits, nx_sent_labels):
    ...
    next_sentence_prediction_loss = tf.reduce_mean(input_tensor=per_example_loss)
    tf.compat.v1.summary.scalar("next_sentence_prediction_loss", next_sentence_prediction_loss)
...
def image_reconstruction_mse_loss(mpm_logits, target_raw_patch_features,
    ...
    image_loss = tf.reduce_mean(input_tensor=image_loss)
    tf.compat.v1.summary.scalar("image_reconstruction_mse_loss", image_loss)
...
def image_reconstruction_kld_loss(mpm_logits, target_raw_patch_features,
    ...
    image_loss = tf.reduce_mean(input_tensor=image_loss)
    tf.compat.v1.summary.scalar("image_reconstruction_kld_loss", image_loss)
